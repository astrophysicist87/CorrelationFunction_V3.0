...
void CorrelationFunction::Set_dN_dypTdpTdphi_moments(FO_surf* FOsurf_ptr, int local_pid)
{
	...
	*global_out_stream_ptr << "Computing weighted spectra..." << endl;
	...
	for (int iqy = 0; iqy < qnpts; ++iqy)
	for (int iqz = 0; iqz < qnpts; ++iqz)
		Cal_dN_dypTdpTdphi_with_weights(FOsurf_ptr, local_pid, 1.0 - 1.e-10, iqt, iqx, iqy, iqz);
	sw.Stop();
...
{
	int giant_array_size = FO_length * eta_s_npts * ntrig;
...
//debugger(__LINE__, __FILE__);
	...
	for (int isurf = 0; isurf < FO_length; ++isurf)
	{
		...
		for (int ieta = 0; ieta < eta_s_npts; ++ieta)
		{
			...
			//cerr << "isurf, ieta = " << isurf << ", " << ieta << endl;
//debugger(__LINE__, __FILE__);
			//iga += 2;
		}
	}
...
/*void CorrelationFunction::Cal_dN_dypTdpTdphi_with_weights(FO_surf* FOsurf_ptr, int local_pid, double cutoff, int iqt, int iqx, int iqy, int iqz)
{
CPStopwatch debug_sw, debug_sw2;
debug_sw.Start();
	int temp_moms_lin_arr_length = n_interp_pT_pts * n_interp_pphi_pts * ntrig;
	int lin_TAA_length = ntrig;
			...
			*global_out_stream_ptr << "\t\t--> Doing pT = " << pT << ", pphi = " << SPinterp_pphi[iphi] << "..." << endl;
			...
			for (int index = 0; index < tmplen; ++index)
			{
				...
				for (int itrig = 0; itrig < ntrig; ++itrig)
				{
					...
					temp_moms_linear_array[lin_TMLAL_idx] += giant_array[lin_TAA_idx] * S_p_withweight;
					++lin_TAA_idx;
					++lin_TMLAL_idx;
				}
...
void CorrelationFunction::Cal_dN_dypTdpTdphi_with_weights(FO_surf* FOsurf_ptr, int local_pid, double cutoff, int iqt, int iqx, int iqy, int iqz)
{
	...
	int temp_moms_lin_arr_length = n_interp_pT_pts * n_interp_pphi_pts * ntrig;
	int lin_TAA_length = eta_s_npts * ntrig;
